...
router.post('/', roleCheck('super_admin', 'admin_comercial', 'backoffice'), async (req, res) => {
  try {
    ...
    const accArr = isSold && Array.isArray(accessories)
      ...
      : null;
    const chType = isSold && ['completa','inscripcion','transferencia','sin_detalle'].includes(charge_type) ? charge_type : null;
...
router.put('/:id', roleCheck('super_admin', 'admin_comercial', 'backoffice', 'vendedor'), asyncHandler(async (req, res) => {
    ...
    if (charge_type !== undefined) {
      ...
      params.push(['completa','inscripcion','transferencia','sin_detalle'].includes(charge_type) ? charge_type : null);
    }
