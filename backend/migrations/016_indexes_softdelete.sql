...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tickets_sla_deadline ON tickets(sla_deadline) WHERE status NOT IN ('ganado','perdido','cerrado');
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reminders_due_date ON reminders(due_date) WHERE completed = false;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_notifications_created_at ON notifications(created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tickets_sla_status ON tickets(sla_status) WHERE status NOT IN ('ganado','perdido','cerrado');
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tickets_deleted_at ON tickets(deleted_at) WHERE deleted_at IS NULL;
