  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg, #FFF7ED 0%, var(--surface) 50%, var(--surface-sunken) 100%)",fontFamily:"'Inter',system-ui,sans-serif",padding:"24px"}}>
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:28}}><img src="/logo.png" alt="MaosBike" style={{height:80,marginBottom:14}}/><p style={{color:"var(--text-subtle)",fontSize:12,marginTop:4}}>Sistema de gestión comercial</p></div>
        <form onSubmit={go} style={{background:"var(--surface)",border:"1px solid var(--surface-sunken)",borderRadius:'var(--radius-xl)',padding:"36px 32px",boxShadow:"0 4px 24px rgba(0,0,0,0.10)"}}>
          <div style={{marginBottom:14}}><label style={S.lbl}>Email</label><input type="email" value={identifier} onChange={e=>setIdentifier(e.target.value)} onKeyDown={e=>e.key==='Enter'&&go(e)} placeholder="tu.correo@maosbike.cl" autoComplete="email" style={{...S.inp,width:"100%"}}/></div>
          <div style={{marginBottom:18}}><label style={S.lbl}>Contraseña</label><div style={{position:"relative"}}><input type={showPw?"text":"password"} value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==='Enter'&&go(e)} autoComplete="current-password" style={{...S.inp,width:"100%",paddingRight:44}}/><button type="button" onClick={()=>setShowPw(s=>!s)} aria-label={showPw?"Ocultar contraseña":"Mostrar contraseña"} title={showPw?"Ocultar contraseña":"Mostrar contraseña"} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"transparent",border:"none",cursor:"pointer",padding:6,color:"var(--text-subtle)",display:"flex",alignItems:"center"}}>{showPw?<Ic.eyeOff size={18}/>:<Ic.eye size={18}/>}</button></div></div>
          <ErrorMsg msg={err}/>
