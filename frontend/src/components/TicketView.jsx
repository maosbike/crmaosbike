...
const slaBox = (bg, border, color) => ({
  ...
  padding:'8px 10px', marginBottom:10, fontSize:11, color,
    ...
    <div style={{ maxWidth:1380 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:14 }}>
        <button onClick={()=>nav("leads")}
          style={{ ...S.gh, display:'flex', alignItems:'center', gap:5, padding:'5px 10px',
            fontSize:12, fontWeight:500, color:'#6B7280', borderRadius:7,
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 308px', gap:14, alignItems:'start' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <div style={{ ...card, padding:'16px 20px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:16 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <h1 style={{ margin:'0 0 6px', fontSize:22, fontWeight:900, color:'#0F172A', letterSpacing:'-0.5px', lineHeight:1.1 }}>
                  {lead.fn} {lead.ln}
                </h1>
                <div style={{ display:'flex', flexWrap:'wrap', gap:10, alignItems:'center' }}>
                  {lead.phone&&(
                    <span style={{ display:'flex', alignItems:'center', gap:4, fontSize:12, color:'#374151', fontWeight:500 }}>
                  ...
                  {lead.comuna&&(
                    <span style={{ fontSize:12, color:'#6B7280' }}>{lead.comuna}</span>
                  )}
                  <span style={{ fontSize:11, color:'#9CA3AF' }}>Ingresó {ago(lead.createdAt)}</span>
                </div>
              </div>
              {/* Meta right */}
              <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6, flexShrink:0 }}>
                ...
                <span style={{ fontSize:11, color:'#9CA3AF' }}>
                  a cargo de <strong style={{ color:'#374151', fontWeight:600 }}>{s.fn} {s.ln}</strong>
                </span>
                {br.name&&(
                  <span style={{ fontSize:10, color:'#9CA3AF', background:'#F3F4F6', padding:'2px 8px', borderRadius:5 }}>
                    {br.code||'MAOS'} · {br.name}
                  </span>
                )}
              </div>
            </div>
          ...
          {/* ── PRODUCTO + CONTACTO — dos columnas compactas ── */}
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>

            {/* Producto Cotizado */}
            <div style={{ ...card, padding:'14px 16px' }}>
              <div style={sectionLabel}>Producto Cotizado</div>
              {m ? (
                <>
                  <div style={{ display:'flex', gap:12, marginBottom:10 }}>
                    {/* Imagen */}
                    <div style={{ width:64, height:52, borderRadius:8, background:'#F3F4F6', overflow:'hidden', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                      {m.image
                    ...
                    {/* Info */}
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:15, fontWeight:800, color:'#0F172A', lineHeight:1.2, marginBottom:2, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                        {m.brand} {m.model}
                      </div>
                      <div style={{ fontSize:11, color:'#9CA3AF', marginBottom:5 }}>
                        {m.year}{m.cc ? ` · ${m.cc}cc` : ''}{m.cat ? ` · ${m.cat}` : ''}
                      </div>
                      {m.price>0 && (
                        <>
                          <div style={{ fontSize:15, fontWeight:900, color:'#F28100', letterSpacing:'-0.3px' }}>
                            Desde {fmt(m.price-m.bonus)}
                          </div>
                          {m.bonus>0 && (
                            <div style={{ fontSize:10, color:'#9CA3AF' }}>{fmt(m.price)} lista</div>
                          )}
                        </>
                      )}
                    </div>
                  ...
                  <div style={{ display:'flex', gap:5, flexWrap:'wrap', marginBottom:10 }}>
                    {lead.colorPref&&(
                      <span style={{ fontSize:10, background:'#F3F4F6', borderRadius:5, padding:'2px 8px', color:'#6B7280', fontWeight:500 }}>
                        Color pref: {lead.colorPref}
                      </span>
                    )}
                    {br.addr&&(
                      <span style={{ fontSize:10, background:'#F3F4F6', borderRadius:5, padding:'2px 8px', color:'#6B7280' }}>
                        📍 {br.addr}
                      </span>
                    )}
                  </div>
                ...
                <div style={{ fontSize:12, color:'#9CA3AF', marginBottom:10, paddingBottom:10 }}>
            ...
            {/* Registrar Contacto */}
            <div style={{ ...card, padding:'14px 16px' }}>
              <div style={sectionLabel}>Registrar Contacto</div>
              <form onSubmit={submitContact}>
                ...
                <select value={contactForm.result} onChange={e=>setContactForm({...contactForm,result:e.target.value})}
                  style={{ ...S.inp, width:'100%', fontSize:11, marginBottom:7 }} required>
                ...
                <textarea value={contactForm.note} onChange={e=>setContactForm({...contactForm,note:e.target.value})}
                  rows={2} style={{ ...S.inp, width:'100%', resize:'none', fontSize:11, marginBottom:7 }}
                  placeholder="Nota / Comentario opcional..."/>
                <button type="submit"
                  style={{ ...S.btn, width:'100%', display:'flex', alignItems:'center', justifyContent:'center', gap:5, fontSize:11, padding:'8px 12px' }}>
              ...
              <div style={{ marginTop:10, paddingTop:10, borderTop:'1px solid #F3F4F6' }}>
                <label style={{ ...S.lbl, fontSize:10 }}>Observaciones Vendedor</label>
                <textarea value={lead.obsVendedor||""} onChange={e=>upd("obsVendedor",e.target.value)} rows={2}
                  style={{ ...S.inp, width:'100%', resize:'none', fontSize:11 }} placeholder="Notas del vendedor..."/>
              </div>
              {isAdmin&&(
                <div style={{ marginTop:6 }}>
                  <label style={{ ...S.lbl, fontSize:10 }}>Observaciones Supervisor</label>
                  <textarea value={lead.obsSupervisor||""} onChange={e=>upd("obsSupervisor",e.target.value)} rows={2}
          ...
          <div style={{ ...card, overflow:'hidden' }}>
            {/* Tab bar */}
            <div style={{ display:'flex', borderBottom:'1px solid #E5E7EB', padding:'0 6px', background:'#FAFAFA' }}>
              {tabs.map(t=>(
                      ...
                      <div style={{ display:'flex', gap:6, marginTop:4 }}>
                        {[true,false].map(v=>(
                          <button key={String(v)} type="button" onClick={()=>upd("wantsFin",v)}
                            style={{ ...S.btn2, padding:'5px 14px', fontSize:12,
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          ...
          <div style={{ ...card, padding:'16px' }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14, paddingBottom:12, borderBottom:'1px solid #F3F4F6' }}>
              <div>
                <div style={{ fontSize:9, fontWeight:800, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'0.12em', marginBottom:5 }}>
                  Ticket #{lead.num}
                </div>
                <TBdg s={lead.status}/>
              </div>
              <div style={{ fontSize:11, color:'#9CA3AF', textAlign:'right' }}>
                {sinContactoH > 0 && (
                  <span style={{ display:'block', fontWeight:600, color: slaBreach ? '#EF4444' : slaWarning ? '#F97316' : '#9CA3AF' }}>
                    {sinContactoH}h sin gestión
                  </span>
                )}
              </div>
            </div>
            ...
            {slaReassigned&&(
              <div style={slaBox("rgba(139,92,246,0.07)","rgba(139,92,246,0.22)","#7C3AED")}>
                ...
                <span><strong>Reasignado</strong> · Este lead fue reasignado automáticamente a {s.fn}{s.ln?` ${s.ln}`:''}. Nuevo SLA activo.</span>
              </div>
            )}
            {slaBreach&&!slaReassigned&&(
              <div style={slaBox("rgba(239,68,68,0.07)","rgba(239,68,68,0.22)","#EF4444")}>
                <Ic.alert size={13} color="#EF4444"/>
                <span><strong>Vencido</strong> · {sinContactoH}h sin gestión · Será reasignado en el próximo ciclo</span>
              </div>
            )}
            {slaWarning&&(
              <div style={slaBox("rgba(249,115,22,0.07)","rgba(249,115,22,0.22)","#F97316")}>
                <Ic.clock size={13} color="#F97316"/>
                <span><strong>Atender ya</strong> · Quedan {8-sinContactoH}h para la reasignación automática</span>
              </div>
            )}

            {/* Prioridad + Estado en grid */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:12 }}>
              ...
              <div>
                <label style={S.lbl}>Marcar como</label>
              ...
              <div style={{ display:'flex', gap:5 }}>
                {[true,false].map(v=>(
                  <button key={String(v)} onClick={()=>upd("testRide",v)}
                    style={{
                      flex:1, padding:'6px', fontSize:11, fontWeight:600, fontFamily:'inherit',
                      borderRadius:7, cursor:'pointer', border:'none',
                      background: lead.testRide===v ? (v?"#10B981":"#374151") : '#F3F4F6',
          ...
          {!isPerdido&&(
            <button onClick={()=>setShowSell(true)}
              style={{
                ...
                background: isGanado ? '#F0FDF4' : '#10B981',
                color: isGanado ? '#10B981' : '#fff',
                border: isGanado ? '1.5px solid #86EFAC' : 'none',
                fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit',
                display:'flex', alignItems:'center', justifyContent:'center', gap:7,
                boxShadow: isGanado ? 'none' : '0 2px 8px rgba(16,185,129,0.3)',
              }}>
              <Ic.sale size={15} color={isGanado?"#10B981":"#fff"}/>
