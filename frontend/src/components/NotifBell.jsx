        ...
        <div style={{position:"fixed",inset:0,zIndex:50}} onClick={()=>setOpen(false)}>
          <div onClick={e=>e.stopPropagation()} className="crm-notif-dropdown" style={{position:"fixed",top:52,right:12,width:340,maxWidth:"calc(100vw - 24px)",maxHeight:400,background:"var(--surface)",border:"1px solid var(--border)",borderRadius:'var(--radius-lg)',boxShadow:"0 8px 24px rgba(0,0,0,0.12)",zIndex:"var(--z-overlay-ui, 200)",overflow:"hidden",display:"flex",flexDirection:"column"}}>
            ...
            <div style={{overflowY:"auto",flex:1}}>
              ...
              {notifs.map(n=>(
                <div key={n.id} onClick={()=>goTicket(n)} style={{padding:"10px 16px",borderBottom:"1px solid var(--surface-muted)",cursor:"pointer",background:n.is_read?"var(--surface)":"var(--brand-soft)",transition:"background 0.1s"}} onMouseEnter={e=>e.currentTarget.style.background=n.is_read?"var(--surface-muted)":"var(--brand-soft)"} onMouseLeave={e=>e.currentTarget.style.background=n.is_read?"var(--surface)":"var(--brand-soft)"}>
