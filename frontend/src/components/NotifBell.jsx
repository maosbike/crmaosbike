  ...
  return(
    <div style={{position:"relative"}}>
      <button onClick={()=>open?setOpen(false):handleOpen()} style={{...S.gh,padding:6,position:"relative"}}>
        <Ic.bell size={17} color={unread>0?"var(--brand)":"#9CA3AF"}/>
        ...
        <div style={{position:"fixed",inset:0,zIndex:50}} onClick={()=>setOpen(false)}>
          <div onClick={e=>e.stopPropagation()} className="crm-notif-dropdown" style={{position:"fixed",top:52,right:12,width:340,maxWidth:"calc(100vw - 24px)",maxHeight:400,background:"#FFFFFF",border:"1px solid #E5E7EB",borderRadius:12,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",zIndex:"var(--z-overlay-ui, 200)",overflow:"hidden",display:"flex",flexDirection:"column"}}>
            ...
            <div style={{overflowY:"auto",flex:1}}>
              ...
              {notifs.map(n=>(
                <div key={n.id} onClick={()=>goTicket(n)} style={{padding:"10px 16px",borderBottom:"1px solid #F9FAFB",cursor:"pointer",background:n.is_read?"#FFFFFF":"var(--brand-soft)",transition:"background 0.1s"}} onMouseEnter={e=>e.currentTarget.style.background=n.is_read?"#F9FAFB":"var(--brand-soft)"} onMouseLeave={e=>e.currentTarget.style.background=n.is_read?"#FFFFFF":"var(--brand-soft)"}>
